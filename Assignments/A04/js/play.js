var play = {
	create: function () {
		...
		this.sound.blast = game.add.audio('blast')
		// Music
	...
	update: function () {
		...
		//this.bmpText.text = game.global.destroy_points
		...
		console.log("obstacle_speed"+ obstacle_speed);
		...
		//game.global.destroy_points +=this.destroyItem(bullet,obstacle);
		//game.global.score += this.destroyItem(bullet, obstacle);
		...
		for (var i = 0; i < obstacles.length; i++) {
			if (obstacles[i].visible) {
				...
				//if (py > oy && Math.abs(py - oy) < 5) {
					//bullet.kill();
					//obstacle.kill();
				if(game.physics.arcade.overlap(game.ufo.bullets, this.obstacles, this.destroyItem, null, this)){
					point += 1;
					if(obstacle_speed > 204.5)
					{
						point += 5;
					}
					if(obstacle_speed > 215)
					{
						point += 10;
					}
